...
void telosbuild::draftproject(string title, name program_manager, asset bond, string description, string github_url, string pdf,
    asset usd_rewarded, uint8_t number_proposals_rewarded, uint32_t proposing_days, uint32_t voting_days) {
    
    validate_ipfs_url(pdf);
...
void telosbuild::newproposal(uint64_t project_id, name proposer, string title, uint64_t number_milestones, string timeline,
    ...
    validate_ipfs_url(timeline);
    validate_ipfs_url(tech_qualifications_pdf);
    validate_ipfs_url(approach_pdf);
    validate_ipfs_url(cost_and_schedule_pdf);
    validate_ipfs_url(references_pdf);
    ...
    proposals.emplace(proposer, [&](auto& col) {
        ...
        col.tech_qualifications_pdf = tech_qualifications_pdf;
        col.approach_pdf = approach_pdf;
        col.cost_and_schedule_pdf = cost_and_schedule_pdf;
        col.references_pdf = references_pdf;
...
void telosbuild::editproposal(uint64_t proposal_id, optional<string> title, optional<string> timeline, optional<uint64_t> number_milestones,
    ...
    string new_tech_qualifications_pdf = proposal.tech_qualifications_pdf;
    if (tech_qualifications_pdf) {
        validate_ipfs_url(*tech_qualifications_pdf);
        new_tech_qualifications_pdf = *tech_qualifications_pdf;
    }

    string new_approach_pdf = proposal.approach_pdf;
    if (approach_pdf) {   
        validate_ipfs_url(*approach_pdf);
        new_approach_pdf = *approach_pdf;
    }

    string new_cost_and_schedule_pdf = proposal.cost_and_schedule_pdf;
    if (cost_and_schedule_pdf) {
        validate_ipfs_url(*cost_and_schedule_pdf);
        new_cost_and_schedule_pdf = *cost_and_schedule_pdf;
    }

    string new_references_pdf = proposal.references_pdf;
    if (references_pdf) {
        validate_ipfs_url(*references_pdf);
        new_references_pdf = *references_pdf;
    }
    ...
    proposals.modify(proposal, same_payer, [&](auto& col) {
        ...
        col.tech_qualifications_pdf = new_tech_qualifications_pdf;
        col.approach_pdf = new_approach_pdf;
        col.cost_and_schedule_pdf = new_cost_and_schedule_pdf;
        col.references_pdf = new_references_pdf;
...
{   
    ...
    if (by_ballot_itr != projects_by_ballot.end()) {
        ...
        projects.modify(*by_ballot_itr, get_self(), [&](auto& col) {
...
void telosbuild::validate_ipfs_url(string ipfs_url)
{
